/* Class321 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

final class Class321 {
    static int anInt3995;
    private int[][] anIntArrayArray3996;
    private int[][] anIntArrayArray3997;
    private int[] anIntArray3998;
    static int anInt3999;
    int anInt4000 = -1;
    private int[] anIntArray4001;
    private int[][] anIntArrayArray4002;
    private int[] anIntArray4003;
    private int[] anIntArray4004;
    static int anInt4005 = -1;
    private IterableHashTable aClass356_4006;
    private int[] anIntArray4007;
    private int[] anIntArray4008;
    private int[] anIntArray4009;
    private String aString4010;
    private String[] aStringArray4011;
    private String aString4012;
    static int anInt4013;
    private int[] anIntArray4014;
    static Class114 aClass114_4015 = new Class114(116, 6);
    private String[] aStringArray4016;
    static int anInt4017;

    final void method2548(byte i) {
        if (i <= 117) method2550(null, -119, (byte) 18);
        anInt3999++;
        if (aString4010 == null) aString4010 = aString4012;
    }

    public static void method2549(byte i) {
        if (i > -74) method2549((byte) 75);
        aClass114_4015 = null;
    }

    private final void method2550(Packet Packet, int i, byte i_0_) {
        if (i_0_ != 52) anIntArray4014 = null;
        anInt4013++;
        if (i != 1) {
            if (i == 2) aString4010 = Packet.method3371(i_0_ ^ ~0x349a);
            else if (i == 3) {
                int i_1_ = Packet.readUnsignedByte(i_0_ ^ 0xcb);
                anIntArrayArray3997 = new int[i_1_][3];
                for (int i_2_ = 0; i_1_ > i_2_; i_2_++) {
                    anIntArrayArray3997[i_2_][0] = Packet.readUnsignedShort(842397944);
                    anIntArrayArray3997[i_2_][1] = Packet.readInt((byte) -126);
                    anIntArrayArray3997[i_2_][2] = Packet.readInt((byte) -126);
                }
            } else if (i == 4) {
                int i_17_ = Packet.readUnsignedByte(255);
                anIntArrayArray3996 = new int[i_17_][3];
                for (int i_18_ = 0; i_17_ > i_18_; i_18_++) {
                    anIntArrayArray3996[i_18_][0] = Packet.readUnsignedShort(842397944);
                    anIntArrayArray3996[i_18_][1] = Packet.readInt((byte) -126);
                    anIntArrayArray3996[i_18_][2] = Packet.readInt((byte) -126);
                }
            } else if (i != 5) {
                if (i == 6) Packet.readUnsignedByte(255);
                else if (i == 7) Packet.readUnsignedByte(255);
                else if (i != 8) {
                    if (i != 9) {
                        if (i == 10) {
                            int i_3_ = Packet.readUnsignedByte(i_0_ + 203);
                            anIntArray4008 = new int[i_3_];
                            for (int i_4_ = 0; i_3_ > i_4_; i_4_++)
                                anIntArray4008[i_4_] = Packet.readInt((byte) -126);
                        } else if (i != 12) {
                            if (i == 13) {
                                int i_15_ = Packet.readUnsignedByte(255);
                                anIntArray4004 = new int[i_15_];
                                for (int i_16_ = 0; i_16_ < i_15_; i_16_++)
                                    anIntArray4004[i_16_] = Packet.readUnsignedShort(842397944);
                            } else if (i == 14) {
                                int i_13_ = Packet.readUnsignedByte(i_0_ ^ 0xcb);
                                anIntArrayArray4002 = new int[i_13_][2];
                                for (int i_14_ = 0; i_14_ < i_13_; i_14_++) {
                                    anIntArrayArray4002[i_14_][0] = Packet.readUnsignedByte(255);
                                    anIntArrayArray4002[i_14_][1] = (Packet.readUnsignedByte(Class348_Sub21.method2955(i_0_, 203)));
                                }
                            } else if (i == 15) Packet.readUnsignedShort(842397944);
                            else if (i == 17) this.anInt4000 = Packet.readUnsignedShort(842397944);
                            else if (i == 18) {
                                int i_5_ = Packet.readUnsignedByte(255);
                                anIntArray4014 = new int[i_5_];
                                aStringArray4011 = new String[i_5_];
                                anIntArray4007 = new int[i_5_];
                                anIntArray4001 = new int[i_5_];
                                for (int i_6_ = 0; (i_6_ < i_5_); i_6_++) {
                                    anIntArray4014[i_6_] = (Packet.readInt((byte) -126));
                                    anIntArray4001[i_6_] = (Packet.readInt((byte) -126));
                                    anIntArray4007[i_6_] = (Packet.readInt((byte) -126));
                                    aStringArray4011[i_6_] = (Packet.readString((byte) 123));
                                }
                            } else if (i == 19) {
                                int i_7_ = Packet.readUnsignedByte(255);
                                aStringArray4016 = new String[i_7_];
                                anIntArray4003 = new int[i_7_];
                                anIntArray4009 = new int[i_7_];
                                anIntArray3998 = new int[i_7_];
                                for (int i_8_ = 0; (i_8_ < i_7_); i_8_++) {
                                    anIntArray4009[i_8_] = (Packet.readInt((byte) -126));
                                    anIntArray3998[i_8_] = (Packet.readInt((byte) -126));
                                    anIntArray4003[i_8_] = (Packet.readInt((byte) -126));
                                    aStringArray4016[i_8_] = (Packet.readString((byte) -25));
                                }
                            } else if (i == 249) {
                                int i_9_ = Packet.readUnsignedByte(255);
                                if (aClass356_4006 == null) {
                                    int i_10_ = (Class33.method340(i_9_, (byte) 108));
                                    aClass356_4006 = new IterableHashTable(i_10_);
                                }
                                for (int i_11_ = 0; i_9_ > i_11_; i_11_++) {
                                    boolean bool = (Packet.readUnsignedByte(255) == 1);
                                    int i_12_ = Packet.readMedium(-1);
                                    Node Node;
                                    if (!bool) Node = (new IntNode(Packet.readInt((byte) -126)));
                                    else Node = (new StringNode(Packet.readString((byte) -98)));
                                    aClass356_4006.method3483((byte) 123, i_12_, Node);
                                }
                            }
                        } else Packet.readInt((byte) -126);
                    } else Packet.readUnsignedByte(255);
                }
            } else Packet.readUnsignedShort(842397944);
        } else aString4012 = Packet.method3371(-13487);
    }

    final void method2551(int i, Packet Packet) {
        int i_19_ = -14 / ((i - -56) / 61);
        anInt3995++;
        for (; ; ) {
            int i_20_ = Packet.readUnsignedByte(255);
            if (i_20_ == 0) break;
            method2550(Packet, i_20_, (byte) 52);
        }
    }

    public Class321() {
        /* empty */
    }
}
