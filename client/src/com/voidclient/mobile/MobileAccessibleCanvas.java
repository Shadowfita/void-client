...
public final class MobileAccessibleCanvas extends AccessibleContext implements AccessibleComponent {
    ...
    private MobileAccessibleCanvas(Canvas c){canvas=new java.lang.ref.WeakReference<>(c);setAccessibleName("Void game interface");setAccessibleDescription("Tap to act, hold for native game actions, drag the scene to rotate. Game operations are revalidated before execution.");}
    ...
    private List<UiFrameSnapshot.Node> nodes(){if(CanvasActionMenu.active())return CanvasActionMenu.nodes();List<UiFrameSnapshot.Node> result=new ArrayList<>();for(UiFrameSnapshot.Node n:MobileBridge.ui().nodes)if(n.visible&&(n.role==UiFrameSnapshot.Role.ITEM||!n.actions.isEmpty()||!n.text.isEmpty()))result.add(n);result.addAll(MobileChrome.nodes());return result;}
    ...
    private final class Node extends AccessibleContext implements Accessible,AccessibleAction,AccessibleComponent {
        ...
        private boolean current(){if(CanvasActionMenu.isNode(node.token))return CanvasActionMenu.current(node);if(CanvasActionMenu.active())return false;if(MobileChrome.isChrome(node.token))return MobileChrome.current(node);UiFrameSnapshot.Node now=MobileBridge.ui().node(node.token);return now!=null&&now.version==node.version&&now.enabled;}
        ...
        public AccessibleComponent getAccessibleComponent(){return this;}
        public int getAccessibleActionCount(){if(CanvasActionMenu.isNode(node.token))return 1;if(MobileChrome.isChrome(node.token))return 1;return node.actions.size()+(node.role==UiFrameSnapshot.Role.TEXT&&node.actions.isEmpty()?0:1);}
        public String getAccessibleActionDescription(int i){if(CanvasActionMenu.isNode(node.token))return i==0?node.label:null;return i==0&&MobileChrome.isChrome(node.token)?node.label:i==0?"Choose native actions":i>0&&i<=node.actions.size()?node.actions.get(i-1).label:null;}
        public boolean doAccessibleAction(int i){if(!isEnabled()||i<0||i>=getAccessibleActionCount())return false;if(CanvasActionMenu.isNode(node.token))return CanvasActionMenu.activate(node);if(MobileChrome.isChrome(node.token))return MobileChrome.activate(node.token,node.version);MobileBridge.nodeAction(i==0?"nodeActions":"nodeOp",i==0?0:node.actions.get(i-1).operation,node.token,node.version);return true;}
