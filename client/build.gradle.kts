...
val mobileNativeNextTest by tasks.registering(JavaExec::class) {
    dependsOn(tasks.testClasses)
    classpath = sourceSets["test"].runtimeClasspath
    mainClass.set("MobileNativeNextRegression")
    jvmArgs("-Djava.awt.headless=true")
}
tasks.test { dependsOn(mobileNativeNextTest) }
val nativeCanvasTest by tasks.registering(JavaExec::class) {
    dependsOn(tasks.testClasses)
    classpath = sourceSets["test"].runtimeClasspath
    mainClass.set("NativeCanvasSmoke")
    jvmArgs("-Djava.awt.headless=false")
}
val releasedNativeCanvasTest by tasks.registering(JavaExec::class) {
    dependsOn(jarRunnerJar, tasks.testClasses)
    classpath = files(sourceSets["test"].output, layout.buildDirectory.file("libs/void-client-jarrunner-mobile.jar"))
    mainClass.set("NativeCanvasSmoke")
    args(layout.buildDirectory.file("proguard/mapping.txt").get().asFile.absolutePath)
