...
public class PatchReader {
  ...
  public PatchApplyPlan readPatchApplyPlan(InputStream in) throws IOException {
    ...
    for (int x = 0; x < numOldFileUncompressionInstructions; x++) {
      long pos = checkNonNegative(dataIn.readLong(), "old file uncompression range pos");
      long length = checkNonNegative(dataIn.readLong(), "old file uncompression range length");
      if (pos < lastReadOffset) {
        throw new PatchFormatException("old file uncompression ranges out of order or overlapping");
      }
      TypedRange<Void> range = new TypedRange<Void>(pos, length, null);
      oldFileUncompressionPlan.add(range);
      lastReadOffset = pos + length; // To check that the next range starts after the current one
    }
    ...
    for (int x = 0; x < numDeltaFriendlyNewFileRecompressionInstructions; x++) {
      long pos = checkNonNegative(
          dataIn.readLong(), "delta-friendly new file recompression range pos");
    ...
    for (int x = 0; x < numDeltaRecords; x++) {
      ...
      long deltaFriendlyOldFileWorkRangeOffset = checkNonNegative(
          dataIn.readLong(), "delta-friendly old file work range pos");
      ...
      long deltaFriendlyNewFileWorkRangeOffset = checkNonNegative(
          dataIn.readLong(), "delta-friendly new file work range pos");
  ...
  /**
   * Assert that the cost isn't negative.
   * @param cost the cost to check
   * @param description the description to use in error messages if the cost is not ok
   * @return the cost
   * @throws PatchFormatException if the cost is not ok
   */
  private static final long checkNonNegative(long cost, String description)
      throws PatchFormatException {
    if (cost < 0) {
      throw new PatchFormatException("Bad cost for " + description + ": " + cost);
    }
    return cost;
  }

  /**
   * Assert that the cost is in the specified range.
   * @param cost the cost to check
   * @param min the minimum (inclusive) cost to allow
   * @param max the maximum (inclusive) cost to allow
   * @param description the description to use in error messages if the cost is not ok
   * @return the cost
   * @throws PatchFormatException if the cost is not ok
   */
  private static final long checkRange(long cost, long min, long max, String description)
      throws PatchFormatException {
