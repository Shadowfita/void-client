...
public class PartiallyCompressingOutputStream extends FilterOutputStream {
  ...
  public void write(byte[] buffer, int pos, int length) throws IOException {
    ...
    while (writtenSoFar < length) {
      writtenSoFar += writeChunk(buffer, pos + writtenSoFar, length - writtenSoFar);
    }
  }
   ...
   * @param pos the pos at which to start copying/compressing
   ...
   */
  private int writeChunk(byte[] buffer, int pos, int length) throws IOException {
    ...
    writeTarget.write(buffer, pos, numBytesToWrite);
